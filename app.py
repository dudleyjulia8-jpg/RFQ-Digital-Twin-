...
with tab3:
    ...
    if st.button("Run Simulation", type="primary"):
        ...
        b2.metric("Avg. Total Cycle Time", f"{scenario['total_cycle_days'].mean():.1f} days",
                   delta=f"{scenario['total_cycle_days'].mean() - baseline['total_cycle_days'].mean():.1f} days",
                   delta_color="inverse")
        high_delay_pct = (scenario["delay_risk"] == "High").mean()
        base_high_delay_pct = (baseline["delay_risk"] == "High").mean()
        b3.metric(
            "% High Delay Risk", f"{high_delay_pct:.0%}",
            delta=f"{(high_delay_pct - base_high_delay_pct):.0%}", delta_color="inverse"
        )
        ...
        with cchart2:
            ...
            st.info(
                f"Under this scenario, **{bottleneck_stage}** takes the longest on average — "
                f"the current constraint in the pipeline."
            )
