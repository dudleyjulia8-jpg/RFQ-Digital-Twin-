...
with tab5:
    ...
    # -----------------------------------------------------------------------
        ...
        row_complexity = selected_row.get("Complexity", None)
        if not row_complexity or pd.isna(row_complexity):
            if models["complexity"]:
                cx_input = pd.DataFrame([{
                    "Industry": selected_row.get("Industry", "Commercial"),
                    "Customer_Tier": selected_row.get("Customer_Tier", "Standard"),
                    "Rough_Value_Estimate": selected_row.get("Rough_Value_Estimate", 50000),
                    ...
                    "Quote_Type": selected_row.get("Quote_Type", "New Quote"),
        ...
        qt_mult = {"New Quote": 1.0, "Repeat Quote": 0.45, "Revision Quote": 0.65}.get(
            selected_row.get("Quote_Type", "New Quote"), 1.0
        )
